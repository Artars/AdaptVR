...
namespace AdaptVR
{
    ...
    public class Hand : MonoBehaviour
    {
        ...
        public HandRole handType;
        [HideInInspector]
        public uint deviceIndex;

        public VivePoseTracker trackedObject;
        
        public ControllerButton grabPinchAction = ControllerButton.Trigger;
        
        public ControllerButton grabGripAction = ControllerButton.Grip;
        
        public ControllerButton uiInteractAction = ControllerButton.Trigger;
        ...
        public bool isPoseValid
        {
            get
            {
                return trackedObject.isPoseValid;
            }
        }
        ...
        {
            // var state = VRModule.GetDeviceState(deviceIndex);

            return VivePose.GetVelocityEx(handType);
        ...
        {
            return VivePose.GetAngularVelocityEx(handType);
        ...
        public void GetEstimatedPeakVelocities(out Vector3 velocity, out Vector3 angularVelocity)
        {
            ...
            velocity = VivePose.GetVelocityEx(handType);
            angularVelocity = VivePose.GetAngularVelocityEx(handType);
        ...
        {
            //Get device index
            deviceIndex = (handType == HandRole.RightHand) ? VRModule.GetRightControllerDeviceIndex() : VRModule.GetLeftControllerDeviceIndex();

            // inputFocusAction = SteamVR_Events.InputFocusAction(OnInputFocus);
            VRModule.onInputFocus += OnInputFocus;
            ...
            if (trackedObject == null)
            {
                trackedObject = this.gameObject.GetComponent<VivePoseTracker>();

                if (trackedObject != null)
                    InvokeRepeating("UpdatePoseTransform", 0, hoverUpdateInterval/10);
            }
        }
        ...
        protected void UpdatePoseTransform()
        {
            if(trackedObject != null)
            {
                OnTransformUpdated(VivePose.GetPoseEx(handType), handType);

            }
        }

        protected virtual void OnTransformUpdated(Pose updatedPose, HandRole updatedSource)
        {
            HandFollowUpdate();
        }
        ...
        private void UpdateDebugText()
        {
            if (showDebugText)
            {
                ...
                if (handType == HandRole.RightHand)
                {
        ...
        public void TriggerHapticPulse(ushort microSecondsDuration)
        {
            ...
            ViveInput.TriggerHapticPulse(handType, microSecondsDuration);
        }
        ...
        public void TriggerHapticPulse(float duration, float frequency, float amplitude)
        {
            ...
            ushort uDur = (ushort) duration;
            ViveInput.TriggerHapticPulse(handType, uDur);
        ...
        public GrabTypes GetGrabStarting(GrabTypes explicitType = GrabTypes.None)
        {
            if (explicitType != GrabTypes.None)
            {
                ...
                if (explicitType == GrabTypes.Pinch && ViveInput.GetPressDownEx(handType, grabPinchAction))
                    return GrabTypes.Pinch;
                if (explicitType == GrabTypes.Grip && ViveInput.GetPressDownEx(handType,grabPinchAction))
            ...
            else
            {
                ...
                if (ViveInput.GetPressDownEx(handType,grabPinchAction))
                    return GrabTypes.Pinch;
                if (ViveInput.GetPressDownEx(handType,grabGripAction))
                    return GrabTypes.Grip;
            }
        ...
        public GrabTypes GetGrabEnding(GrabTypes explicitType = GrabTypes.None)
        {
            if (explicitType != GrabTypes.None)
            {
                ...
                if (explicitType == GrabTypes.Pinch && ViveInput.GetPressUpEx(handType,grabPinchAction))
                    return GrabTypes.Pinch;
                if (explicitType == GrabTypes.Grip && ViveInput.GetPressUpEx(handType,grabGripAction))
                    return GrabTypes.Grip;
            }
            else
            {
                ...
                if (ViveInput.GetPressUpEx(handType,grabPinchAction))
                    return GrabTypes.Pinch;
                if (ViveInput.GetPressUpEx(handType,grabGripAction))
                    return GrabTypes.Grip;
            }
        ...
        public bool IsGrabbingWithType(GrabTypes type)
        {
            ...
            switch (type)
            {
                case GrabTypes.Pinch:
                    return ViveInput.GetPressEx(handType,grabPinchAction);

                case GrabTypes.Grip:
                    return ViveInput.GetPressEx(handType,grabGripAction);
        ...
        public GrabTypes GetBestGrabbingType(GrabTypes preferred, bool forcePreference = false)
        {
            ...
            {
                if (ViveInput.GetPressEx(handType,grabPinchAction))
                    return GrabTypes.Pinch;
            ...
            {
                if (ViveInput.GetPressEx(handType,grabGripAction))
                    return GrabTypes.Grip;
                else if (forcePreference)
                    return GrabTypes.None;
            }
        ...
        private void InitController()
        {
            ...
            var deviceIndex = trackedObject.viveRole;
