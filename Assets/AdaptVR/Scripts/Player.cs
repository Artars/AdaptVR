...
namespace AdaptVR
{
	...
	public class Player : MonoBehaviour
	{
        ...
        public ControllerButton headsetOnHead = ControllerButton.ProximitySensor;
		...
		public Hand leftHand
		{
			get
			{
				for ( int j = 0; j < hands.Length; j++ )
				{
					...
					if ( hands[j].handType != HandRole.LeftHand)
					{
						continue;
					}
		...
		public Hand rightHand
		{
			get
			{
				for ( int j = 0; j < hands.Length; j++ )
				{
					...
					if ( hands[j].handType != HandRole.RightHand)
					{
						continue;
					}
...
// 			}
        ...
        protected virtual void Update()
        {
            ...
            {
                if (ViveInput.GetPressDownEx(BodyRole.Head, headsetOnHead))
                {
                    Debug.Log("<b>SteamVR Interaction System</b> Headset placed on head");
                }
                else if (ViveInput.GetPressUpEx(BodyRole.Head, headsetOnHead))
                {
